<?php

use Livewire\Volt\Component;
use Livewire\Attributes\Layout;

new #[Layout("layouts.frontend")] class extends Component {
    //
}; ?>

<div>
    <section class="overflow-hidden pb-[120px] pt-[180px]">
        <div class="container">
            <div class="-mx-4 flex flex-wrap">
                <div class="w-full px-4 lg:w-8/12">
                    <div>
                        <h2
                            class="mb-8 text-3xl font-bold leading-tight text-black dark:text-white sm:text-4xl sm:leading-tight"
                        >
                            10 amazing sites to download stock photos &amp;
                            digital assets for free
                        </h2>
                        <div
                            class="border-body-color mb-10 flex flex-wrap items-center justify-between border-b border-opacity-10 pb-4 dark:border-white dark:border-opacity-10"
                        >
                            <div class="flex flex-wrap items-center">
                                <div class="mb-5 mr-10 flex items-center">
                                    <div class="mr-4">
                                        <div
                                            class="relative h-10 w-10 overflow-hidden rounded-full"
                                        >
                                            <img
                                                alt="author"
                                                sizes="100vw"
                                                srcset="
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=640&amp;q=75   640w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=750&amp;q=75   750w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=828&amp;q=75   828w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=1080&amp;q=75 1080w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=1200&amp;q=75 1200w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=1920&amp;q=75 1920w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=2048&amp;q=75 2048w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=3840&amp;q=75 3840w
                                                "
                                                src="../_next/image/indexe8fb.html?url=%2Fimages%2Fblog%2Fauthor-02.png&amp;w=3840&amp;q=75"
                                                decoding="async"
                                                data-nimg="fill"
                                                loading="lazy"
                                                style="
                                                    position: absolute;
                                                    height: 100%;
                                                    width: 100%;
                                                    left: 0;
                                                    top: 0;
                                                    right: 0;
                                                    bottom: 0;
                                                    color: transparent;
                                                "
                                            />
                                        </div>
                                    </div>
                                    <div class="w-full">
                                        <h4
                                            class="text-body-color mb-1 text-base font-medium"
                                        >
                                            By
                                            <span>Musharof Chy</span>
                                        </h4>
                                    </div>
                                </div>
                                <div class="mb-5 flex items-center">
                                    <p
                                        class="text-body-color mr-5 flex items-center text-base font-medium"
                                    >
                                        <span class="mr-3">
                                            <svg
                                                width="15"
                                                height="15"
                                                viewBox="0 0 15 15"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M3.89531 8.67529H3.10666C2.96327 8.67529 2.86768 8.77089 2.86768 8.91428V9.67904C2.86768 9.82243 2.96327 9.91802 3.10666 9.91802H3.89531C4.03871 9.91802 4.1343 9.82243 4.1343 9.67904V8.91428C4.1343 8.77089 4.03871 8.67529 3.89531 8.67529Z"
                                                ></path>
                                                <path
                                                    d="M6.429 8.67529H5.64035C5.49696 8.67529 5.40137 8.77089 5.40137 8.91428V9.67904C5.40137 9.82243 5.49696 9.91802 5.64035 9.91802H6.429C6.57239 9.91802 6.66799 9.82243 6.66799 9.67904V8.91428C6.66799 8.77089 6.5485 8.67529 6.429 8.67529Z"
                                                ></path>
                                                <path
                                                    d="M8.93828 8.67529H8.14963C8.00624 8.67529 7.91064 8.77089 7.91064 8.91428V9.67904C7.91064 9.82243 8.00624 9.91802 8.14963 9.91802H8.93828C9.08167 9.91802 9.17727 9.82243 9.17727 9.67904V8.91428C9.17727 8.77089 9.08167 8.67529 8.93828 8.67529Z"
                                                ></path>
                                                <path
                                                    d="M11.4715 8.67529H10.6828C10.5394 8.67529 10.4438 8.77089 10.4438 8.91428V9.67904C10.4438 9.82243 10.5394 9.91802 10.6828 9.91802H11.4715C11.6149 9.91802 11.7105 9.82243 11.7105 9.67904V8.91428C11.7105 8.77089 11.591 8.67529 11.4715 8.67529Z"
                                                ></path>
                                                <path
                                                    d="M3.89531 11.1606H3.10666C2.96327 11.1606 2.86768 11.2562 2.86768 11.3996V12.1644C2.86768 12.3078 2.96327 12.4034 3.10666 12.4034H3.89531C4.03871 12.4034 4.1343 12.3078 4.1343 12.1644V11.3996C4.1343 11.2562 4.03871 11.1606 3.89531 11.1606Z"
                                                ></path>
                                                <path
                                                    d="M6.429 11.1606H5.64035C5.49696 11.1606 5.40137 11.2562 5.40137 11.3996V12.1644C5.40137 12.3078 5.49696 12.4034 5.64035 12.4034H6.429C6.57239 12.4034 6.66799 12.3078 6.66799 12.1644V11.3996C6.66799 11.2562 6.5485 11.1606 6.429 11.1606Z"
                                                ></path>
                                                <path
                                                    d="M8.93828 11.1606H8.14963C8.00624 11.1606 7.91064 11.2562 7.91064 11.3996V12.1644C7.91064 12.3078 8.00624 12.4034 8.14963 12.4034H8.93828C9.08167 12.4034 9.17727 12.3078 9.17727 12.1644V11.3996C9.17727 11.2562 9.08167 11.1606 8.93828 11.1606Z"
                                                ></path>
                                                <path
                                                    d="M11.4715 11.1606H10.6828C10.5394 11.1606 10.4438 11.2562 10.4438 11.3996V12.1644C10.4438 12.3078 10.5394 12.4034 10.6828 12.4034H11.4715C11.6149 12.4034 11.7105 12.3078 11.7105 12.1644V11.3996C11.7105 11.2562 11.591 11.1606 11.4715 11.1606Z"
                                                ></path>
                                                <path
                                                    d="M13.2637 3.3697H7.64754V2.58105C8.19721 2.43765 8.62738 1.91189 8.62738 1.31442C8.62738 0.597464 8.02992 0 7.28906 0C6.54821 0 5.95074 0.597464 5.95074 1.31442C5.95074 1.91189 6.35702 2.41376 6.93058 2.58105V3.3697H1.31442C0.597464 3.3697 0 3.96716 0 4.68412V13.2637C0 13.9807 0.597464 14.5781 1.31442 14.5781H13.2637C13.9807 14.5781 14.5781 13.9807 14.5781 13.2637V4.68412C14.5781 3.96716 13.9807 3.3697 13.2637 3.3697ZM6.6677 1.31442C6.6677 0.979841 6.93058 0.716957 7.28906 0.716957C7.62364 0.716957 7.91042 0.979841 7.91042 1.31442C7.91042 1.649 7.64754 1.91189 7.28906 1.91189C6.95448 1.91189 6.6677 1.6251 6.6677 1.31442ZM1.31442 4.08665H13.2637C13.5983 4.08665 13.8612 4.34954 13.8612 4.68412V6.45261H0.716957V4.68412C0.716957 4.34954 0.979841 4.08665 1.31442 4.08665ZM13.2637 13.8612H1.31442C0.979841 13.8612 0.716957 13.5983 0.716957 13.2637V7.16957H13.8612V13.2637C13.8612 13.5983 13.5983 13.8612 13.2637 13.8612Z"
                                                ></path>
                                            </svg>
                                        </span>
                                        12 Jan 2024
                                    </p>
                                    <p
                                        class="text-body-color mr-5 flex items-center text-base font-medium"
                                    >
                                        <span class="mr-3">
                                            <svg
                                                width="18"
                                                height="13"
                                                viewBox="0 0 18 13"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M15.6375 0H1.6875C0.759375 0 0 0.759375 0 1.6875V10.6875C0 11.3062 0.309375 11.8406 0.84375 12.15C1.09687 12.2906 1.40625 12.375 1.6875 12.375C1.96875 12.375 2.25 12.2906 2.53125 12.15L5.00625 10.7156C5.11875 10.6594 5.23125 10.6312 5.34375 10.6312H15.6094C16.5375 10.6312 17.2969 9.87187 17.2969 8.94375V1.6875C17.325 0.759375 16.5656 0 15.6375 0ZM16.3406 8.94375C16.3406 9.3375 16.0312 9.64687 15.6375 9.64687H5.37187C5.09062 9.64687 4.78125 9.73125 4.52812 9.87187L2.05313 11.3063C1.82812 11.4187 1.575 11.4187 1.35 11.3063C1.125 11.1938 1.0125 10.9688 1.0125 10.7156V1.6875C1.0125 1.29375 1.32188 0.984375 1.71563 0.984375H15.6656C16.0594 0.984375 16.3687 1.29375 16.3687 1.6875V8.94375H16.3406Z"
                                                ></path>
                                                <path
                                                    d="M12.2342 3.375H4.69668C4.41543 3.375 4.19043 3.6 4.19043 3.88125C4.19043 4.1625 4.41543 4.3875 4.69668 4.3875H12.2623C12.5435 4.3875 12.7685 4.1625 12.7685 3.88125C12.7685 3.6 12.5154 3.375 12.2342 3.375Z"
                                                ></path>
                                                <path
                                                    d="M11.0529 6.55322H4.69668C4.41543 6.55322 4.19043 6.77822 4.19043 7.05947C4.19043 7.34072 4.41543 7.56572 4.69668 7.56572H11.0811C11.3623 7.56572 11.5873 7.34072 11.5873 7.05947C11.5873 6.77822 11.3342 6.55322 11.0529 6.55322Z"
                                                ></path>
                                            </svg>
                                        </span>
                                        50
                                    </p>
                                    <p
                                        class="text-body-color flex items-center text-base font-medium"
                                    >
                                        <span class="mr-3">
                                            <svg
                                                width="20"
                                                height="12"
                                                viewBox="0 0 20 12"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M10.2559 3.8125C9.03711 3.8125 8.06836 4.8125 8.06836 6C8.06836 7.1875 9.06836 8.1875 10.2559 8.1875C11.4434 8.1875 12.4434 7.1875 12.4434 6C12.4434 4.8125 11.4746 3.8125 10.2559 3.8125ZM10.2559 7.09375C9.66211 7.09375 9.16211 6.59375 9.16211 6C9.16211 5.40625 9.66211 4.90625 10.2559 4.90625C10.8496 4.90625 11.3496 5.40625 11.3496 6C11.3496 6.59375 10.8496 7.09375 10.2559 7.09375Z"
                                                ></path>
                                                <path
                                                    d="M19.7559 5.625C17.6934 2.375 14.1309 0.4375 10.2559 0.4375C6.38086 0.4375 2.81836 2.375 0.755859 5.625C0.630859 5.84375 0.630859 6.125 0.755859 6.34375C2.81836 9.59375 6.38086 11.5312 10.2559 11.5312C14.1309 11.5312 17.6934 9.59375 19.7559 6.34375C19.9121 6.125 19.9121 5.84375 19.7559 5.625ZM10.2559 10.4375C6.84961 10.4375 3.69336 8.78125 1.81836 5.96875C3.69336 3.1875 6.84961 1.53125 10.2559 1.53125C13.6621 1.53125 16.8184 3.1875 18.6934 5.96875C16.8184 8.78125 13.6621 10.4375 10.2559 10.4375Z"
                                                ></path>
                                            </svg>
                                        </span>
                                        35
                                    </p>
                                </div>
                            </div>
                            <div class="mb-5">
                                <a
                                    href="#0"
                                    class="bg-primary inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-semibold text-white"
                                >
                                    Design
                                </a>
                            </div>
                        </div>
                        <div>
                            <p
                                class="text-body-color mb-10 text-base font-medium leading-relaxed sm:text-lg sm:leading-relaxed lg:text-base lg:leading-relaxed xl:text-lg xl:leading-relaxed"
                            >
                                Duis aute irure dolor in reprehenderit in
                                voluptate velit esse cillum dolore eu fugiat
                                nulla pariatur. Excepteur sint occaecat
                                cupidatat.
                            </p>
                            <div class="mb-10 w-full overflow-hidden rounded">
                                <div
                                    class="relative aspect-[97/60] w-full sm:aspect-[97/44]"
                                >
                                    <img
                                        alt="image"
                                        sizes="100vw"
                                        srcset="
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=640&amp;q=75   640w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=750&amp;q=75   750w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=828&amp;q=75   828w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=1080&amp;q=75 1080w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=1200&amp;q=75 1200w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=1920&amp;q=75 1920w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=2048&amp;q=75 2048w,
                                            /_next/image/?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=3840&amp;q=75 3840w
                                        "
                                        src="../_next/image/index768b.html?url=%2Fimages%2Fblog%2Fblog-details-01.jpg&amp;w=3840&amp;q=75"
                                        decoding="async"
                                        data-nimg="fill"
                                        class="h-full w-full object-cover object-center"
                                        loading="lazy"
                                        style="
                                            position: absolute;
                                            height: 100%;
                                            width: 100%;
                                            left: 0;
                                            top: 0;
                                            right: 0;
                                            bottom: 0;
                                            color: transparent;
                                        "
                                    />
                                </div>
                            </div>
                            <p
                                class="text-body-color mb-8 text-base font-medium leading-relaxed sm:text-lg sm:leading-relaxed lg:text-base lg:leading-relaxed xl:text-lg xl:leading-relaxed"
                            >
                                Lorem ipsum dolor sit amet, consectetur
                                adipiscing elit, sed do eiusmod tempor
                                incididunt ut labore et dolore magna aliqua.
                                Quis enim lobortis scelerisque fermentum. Neque
                                sodales ut etiam sit amet. Ligula ullamcorper
                                <strong class="text-primary dark:text-white">
                                    <!-- -->
                                    malesuada
                                    <!-- -->
                                </strong>
                                proin libero nunc consequat interdum varius.
                                Quam pellentesque nec nam aliquam sem et tortor
                                consequat. Pellentesque adipiscing commodo elit
                                at imperdiet.
                            </p>
                            <p
                                class="text-body-color mb-10 text-base font-medium leading-relaxed sm:text-lg sm:leading-relaxed lg:text-base lg:leading-relaxed xl:text-lg xl:leading-relaxed"
                            >
                                Semper auctor neque vitae tempus quam
                                pellentesque nec.
                                <span
                                    class="text-primary underline dark:text-white"
                                >
                                    <!-- -->
                                    Amet dictum sit amet justo
                                    <!-- -->
                                </span>
                                donec enim diam. Varius sit amet mattis
                                vulputate enim nulla aliquet porttitor. Odio
                                pellentesque diam volutpat commodo sed.
                            </p>
                            <h3
                                class="font-xl mb-10 font-bold leading-tight text-black dark:text-white sm:text-2xl sm:leading-tight lg:text-xl lg:leading-tight xl:text-2xl xl:leading-tight"
                            >
                                Digital marketplace for Ui/Ux designers.
                            </h3>
                            <p
                                class="text-body-color mb-10 text-base font-medium leading-relaxed sm:text-lg sm:leading-relaxed lg:text-base lg:leading-relaxed xl:text-lg xl:leading-relaxed"
                            >
                                consectetur adipiscing elit in voluptate velit
                                esse cillum dolore eu fugiat nulla pariatur.
                                Excepteur sint occaecat mattis vulputate
                                cupidatat.
                            </p>
                            <ul
                                class="text-body-color mb-10 list-inside list-disc"
                            >
                                <li
                                    class="text-body-color mb-2 text-base font-medium sm:text-lg lg:text-base xl:text-lg"
                                >
                                    Consectetur adipiscing elit in voluptate
                                    velit.
                                </li>
                                <li
                                    class="text-body-color mb-2 text-base font-medium sm:text-lg lg:text-base xl:text-lg"
                                >
                                    Mattis vulputate cupidatat.
                                </li>
                                <li
                                    class="text-body-color mb-2 text-base font-medium sm:text-lg lg:text-base xl:text-lg"
                                >
                                    Vulputate enim nulla aliquet porttitor odio
                                    pellentesque
                                </li>
                                <li
                                    class="text-body-color mb-2 text-base font-medium sm:text-lg lg:text-base xl:text-lg"
                                >
                                    Ligula ullamcorper malesuada proin
                                </li>
                            </ul>
                            <div
                                class="bg-primary relative z-10 mb-10 overflow-hidden rounded-md bg-opacity-10 p-8 md:p-9 lg:p-8 xl:p-9"
                            >
                                <p
                                    class="text-body-color text-center text-base font-medium italic"
                                >
                                    Lorem ipsum dolor sit amet, consectetur
                                    adipiscing elit, sed do eiusmod incididunt
                                    utionals labore et dolore magna aliqua. Quis
                                    lobortis scelerisque fermentum, The Neque ut
                                    etiam sit amet.
                                </p>
                                <span class="absolute left-0 top-0 z-[-1]">
                                    <svg
                                        width="132"
                                        height="109"
                                        viewBox="0 0 132 109"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <path
                                            opacity="0.5"
                                            d="M33.0354 90.11C19.9851 102.723 -3.75916 101.834 -14 99.8125V-15H132C131.456 -12.4396 127.759 -2.95278 117.318 14.5117C104.268 36.3422 78.7114 31.8952 63.2141 41.1934C47.7169 50.4916 49.3482 74.3435 33.0354 90.11Z"
                                            fill="url(#paint0_linear_111:606)"
                                        ></path>
                                        <path
                                            opacity="0.5"
                                            d="M33.3654 85.0768C24.1476 98.7862 1.19876 106.079 -9.12343 108.011L-38.876 22.9988L100.816 -25.8905C100.959 -23.8126 99.8798 -15.5499 94.4164 0.87754C87.5871 21.4119 61.9822 26.677 49.5641 38.7512C37.146 50.8253 44.8877 67.9401 33.3654 85.0768Z"
                                            fill="url(#paint1_linear_111:606)"
                                        ></path>
                                        <defs>
                                            <linearGradient
                                                id="paint0_linear_111:606"
                                                x1="94.7523"
                                                y1="82.0246"
                                                x2="8.40951"
                                                y2="52.0609"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop
                                                    stop-color="white"
                                                    stop-opacity="0.06"
                                                ></stop>
                                                <stop
                                                    offset="1"
                                                    stop-color="white"
                                                    stop-opacity="0"
                                                ></stop>
                                            </linearGradient>
                                            <linearGradient
                                                id="paint1_linear_111:606"
                                                x1="90.3206"
                                                y1="58.4236"
                                                x2="1.16149"
                                                y2="50.8365"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop
                                                    stop-color="white"
                                                    stop-opacity="0.06"
                                                ></stop>
                                                <stop
                                                    offset="1"
                                                    stop-color="white"
                                                    stop-opacity="0"
                                                ></stop>
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </span>
                                <span class="absolute bottom-0 right-0 z-[-1]">
                                    <svg
                                        width="53"
                                        height="30"
                                        viewBox="0 0 53 30"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <circle
                                            opacity="0.8"
                                            cx="37.5"
                                            cy="37.5"
                                            r="37.5"
                                            fill="#4A6CF7"
                                        ></circle>
                                        <mask
                                            id="mask0_111:596"
                                            style="mask-type: alpha"
                                            maskUnits="userSpaceOnUse"
                                            x="0"
                                            y="0"
                                            width="75"
                                            height="75"
                                        >
                                            <circle
                                                opacity="0.8"
                                                cx="37.5"
                                                cy="37.5"
                                                r="37.5"
                                                fill="#4A6CF7"
                                            ></circle>
                                        </mask>
                                        <g mask="url(#mask0_111:596)">
                                            <circle
                                                opacity="0.8"
                                                cx="37.5"
                                                cy="37.5"
                                                r="37.5"
                                                fill="url(#paint0_radial_111:596)"
                                            ></circle>
                                            <g
                                                opacity="0.8"
                                                filter="url(#filter0_f_111:596)"
                                            >
                                                <circle
                                                    cx="40.8089"
                                                    cy="19.853"
                                                    r="15.4412"
                                                    fill="white"
                                                ></circle>
                                            </g>
                                        </g>
                                        <defs>
                                            <filter
                                                id="filter0_f_111:596"
                                                x="4.36768"
                                                y="-16.5881"
                                                width="72.8823"
                                                height="72.8823"
                                                filterUnits="userSpaceOnUse"
                                                color-interpolation-filters="sRGB"
                                            >
                                                <feFlood
                                                    flood-opacity="0"
                                                    result="BackgroundImageFix"
                                                ></feFlood>
                                                <feBlend
                                                    mode="normal"
                                                    in="SourceGraphic"
                                                    in2="BackgroundImageFix"
                                                    result="shape"
                                                ></feBlend>
                                                <feGaussianBlur
                                                    stdDeviation="10.5"
                                                    result="effect1_foregroundBlur_111:596"
                                                ></feGaussianBlur>
                                            </filter>
                                            <radialGradient
                                                id="paint0_radial_111:596"
                                                cx="0"
                                                cy="0"
                                                r="1"
                                                gradientUnits="userSpaceOnUse"
                                                gradientTransform="translate(37.5 37.5) rotate(90) scale(40.2574)"
                                            >
                                                <stop
                                                    stop-opacity="0.47"
                                                ></stop>
                                                <stop
                                                    offset="1"
                                                    stop-opacity="0"
                                                ></stop>
                                            </radialGradient>
                                        </defs>
                                    </svg>
                                </span>
                            </div>
                            <p
                                class="text-body-color mb-10 text-base font-medium leading-relaxed sm:text-lg sm:leading-relaxed lg:text-base lg:leading-relaxed xl:text-lg xl:leading-relaxed"
                            >
                                consectetur adipiscing elit in voluptate velit
                                esse cillum dolore eu fugiat nulla pariatur.
                                Excepteur sint occaecat mattis vulputate
                                cupidatat.
                            </p>
                            <div class="items-center justify-between sm:flex">
                                <div class="mb-5">
                                    <h5
                                        class="text-body-color mb-3 text-sm font-medium"
                                    >
                                        Popular Tags :
                                    </h5>
                                    <div class="flex items-center">
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                                        >
                                            Design
                                        </a>
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                                        >
                                            Development
                                        </a>
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                                        >
                                            Info
                                        </a>
                                    </div>
                                </div>
                                <div class="mb-5">
                                    <h5
                                        class="text-body-color mb-3 text-sm font-medium sm:text-right"
                                    >
                                        Share this post :
                                    </h5>
                                    <div
                                        class="flex items-center sm:justify-end"
                                    >
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 inline-flex h-9 w-9 items-center justify-center rounded-md bg-opacity-10 duration-300 hover:bg-opacity-100 hover:text-white sm:ml-3"
                                        >
                                            <svg
                                                width="16"
                                                height="16"
                                                viewBox="0 0 16 16"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M14.3442 0H1.12455C0.499798 0 0 0.497491 0 1.11936V14.3029C0 14.8999 0.499798 15.4222 1.12455 15.4222H14.2942C14.919 15.4222 15.4188 14.9247 15.4188 14.3029V1.09448C15.4688 0.497491 14.969 0 14.3442 0ZM4.57316 13.1089H2.29907V5.7709H4.57316V13.1089ZM3.42362 4.75104C2.67392 4.75104 2.09915 4.15405 2.09915 3.43269C2.09915 2.71133 2.69891 2.11434 3.42362 2.11434C4.14833 2.11434 4.74809 2.71133 4.74809 3.43269C4.74809 4.15405 4.19831 4.75104 3.42362 4.75104ZM13.1947 13.1089H10.9206V9.55183C10.9206 8.7061 10.8956 7.58674 9.72108 7.58674C8.52156 7.58674 8.34663 8.53198 8.34663 9.47721V13.1089H6.07255V5.7709H8.29665V6.79076H8.32164C8.64651 6.19377 9.37122 5.59678 10.4958 5.59678C12.8198 5.59678 13.2447 7.08925 13.2447 9.12897V13.1089H13.1947Z"
                                                ></path>
                                            </svg>
                                        </a>
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 ml-3 inline-flex h-9 w-9 items-center justify-center rounded-md bg-opacity-10 duration-300 hover:bg-opacity-100 hover:text-white"
                                        >
                                            <svg
                                                width="18"
                                                height="14"
                                                viewBox="0 0 18 14"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M15.5524 2.26027L16.625 1.0274C16.9355 0.693493 17.0202 0.436644 17.0484 0.308219C16.2016 0.770548 15.4113 0.924658 14.9032 0.924658H14.7056L14.5927 0.821918C13.9153 0.282534 13.0685 0 12.1653 0C10.1895 0 8.6371 1.48973 8.6371 3.21062C8.6371 3.31336 8.6371 3.46747 8.66532 3.57021L8.75 4.0839L8.15726 4.05822C4.54435 3.95548 1.58065 1.13014 1.10081 0.642123C0.310484 1.92637 0.762097 3.15925 1.24194 3.92979L2.20161 5.36815L0.677419 4.5976C0.705645 5.67637 1.15726 6.52397 2.03226 7.14041L2.79435 7.65411L2.03226 7.93665C2.5121 9.24658 3.58468 9.78596 4.375 9.99144L5.41935 10.2483L4.43145 10.8647C2.85081 11.8921 0.875 11.8151 0 11.738C1.77823 12.8682 3.89516 13.125 5.3629 13.125C6.46371 13.125 7.28226 13.0223 7.47984 12.9452C15.3831 11.25 15.75 4.82877 15.75 3.54452V3.36473L15.9194 3.26199C16.879 2.44007 17.2742 2.00342 17.5 1.74658C17.4153 1.77226 17.3024 1.82363 17.1895 1.84932L15.5524 2.26027Z"
                                                ></path>
                                            </svg>
                                        </a>
                                        <a
                                            href="#0"
                                            class="bg-primary text-body-color mb-3 ml-3 inline-flex h-9 w-9 items-center justify-center rounded-md bg-opacity-10 duration-300 hover:bg-opacity-100 hover:text-white"
                                        >
                                            <svg
                                                width="9"
                                                height="18"
                                                viewBox="0 0 9 18"
                                                class="fill-current"
                                            >
                                                <path
                                                    d="M8.13643 7H6.78036H6.29605V6.43548V4.68548V4.12097H6.78036H7.79741C8.06378 4.12097 8.28172 3.89516 8.28172 3.55645V0.564516C8.28172 0.254032 8.088 0 7.79741 0H6.02968C4.11665 0 2.78479 1.58064 2.78479 3.92339V6.37903V6.94355H2.30048H0.65382C0.314802 6.94355 0 7.25403 0 7.70564V9.7379C0 10.1331 0.266371 10.5 0.65382 10.5H2.25205H2.73636V11.0645V16.7379C2.73636 17.1331 3.00273 17.5 3.39018 17.5H5.66644C5.81174 17.5 5.93281 17.4153 6.02968 17.3024C6.12654 17.1895 6.19919 16.9919 6.19919 16.8226V11.0927V10.5282H6.70771H7.79741C8.11222 10.5282 8.35437 10.3024 8.4028 9.96371V9.93548V9.90726L8.74182 7.95968C8.76604 7.7621 8.74182 7.53629 8.59653 7.31048C8.54809 7.16935 8.33016 7.02823 8.13643 7Z"
                                                ></path>
                                            </svg>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-full px-4 lg:w-4/12">
                    <div
                        class="bg-primary mb-10 mt-12 rounded-md bg-opacity-5 p-6 dark:bg-opacity-5 lg:mt-0"
                    >
                        <form class="flex items-center justify-between">
                            <input
                                type="text"
                                placeholder="Search here..."
                                class="palceholder-body-color text-body-color focus:border-primary mr-5 w-full rounded-md border border-transparent px-5 py-3 text-base font-medium outline-none dark:bg-white dark:bg-opacity-10"
                            />
                            <button
                                class="bg-primary flex h-[50px] w-full max-w-[50px] items-center justify-center rounded-md text-white"
                            >
                                <svg
                                    width="20"
                                    height="18"
                                    viewBox="0 0 20 18"
                                    fill="none"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M19.4062 16.8125L13.9375 12.375C14.9375 11.0625 15.5 9.46875 15.5 7.78125C15.5 5.75 14.7188 3.875 13.2812 2.4375C10.3438 -0.5 5.5625 -0.5 2.59375 2.4375C1.1875 3.84375 0.40625 5.75 0.40625 7.75C0.40625 9.78125 1.1875 11.6562 2.625 13.0937C4.09375 14.5625 6.03125 15.3125 7.96875 15.3125C9.875 15.3125 11.75 14.5938 13.2188 13.1875L18.75 17.6562C18.8438 17.75 18.9688 17.7812 19.0938 17.7812C19.25 17.7812 19.4062 17.7188 19.5312 17.5938C19.6875 17.3438 19.6562 17 19.4062 16.8125ZM3.375 12.3438C2.15625 11.125 1.5 9.5 1.5 7.75C1.5 6 2.15625 4.40625 3.40625 3.1875C4.65625 1.9375 6.3125 1.3125 7.96875 1.3125C9.625 1.3125 11.2812 1.9375 12.5312 3.1875C13.75 4.40625 14.4375 6.03125 14.4375 7.75C14.4375 9.46875 13.7188 11.125 12.5 12.3438C10 14.8438 5.90625 14.8438 3.375 12.3438Z"
                                        fill="white"
                                    ></path>
                                </svg>
                            </button>
                        </form>
                    </div>
                    <div
                        class="bg-primary mb-10 rounded-md bg-opacity-5 dark:bg-opacity-10"
                    >
                        <h3
                            class="border-body-color border-b border-opacity-10 px-8 py-4 text-lg font-semibold text-black dark:border-white dark:border-opacity-10 dark:text-white"
                        >
                            Related Posts
                        </h3>
                        <ul class="p-8">
                            <li
                                class="border-body-color mb-6 border-b border-opacity-10 pb-6 dark:border-white dark:border-opacity-10"
                            >
                                <div class="flex items-center lg:block xl:flex">
                                    <div class="mr-5 lg:mb-3 xl:mb-0">
                                        <div
                                            class="relative h-[60px] w-[70px] overflow-hidden rounded-md sm:h-[75px] sm:w-[85px]"
                                        >
                                            <img
                                                alt="Best way to boost your online sales."
                                                sizes="100vw"
                                                srcset="
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=640&amp;q=75   640w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=750&amp;q=75   750w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=828&amp;q=75   828w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=1080&amp;q=75 1080w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=1200&amp;q=75 1200w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=1920&amp;q=75 1920w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=2048&amp;q=75 2048w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=3840&amp;q=75 3840w
                                                "
                                                src="../_next/image/index78c1.html?url=%2Fimages%2Fblog%2Fpost-01.jpg&amp;w=3840&amp;q=75"
                                                decoding="async"
                                                data-nimg="fill"
                                                loading="lazy"
                                                style="
                                                    position: absolute;
                                                    height: 100%;
                                                    width: 100%;
                                                    left: 0;
                                                    top: 0;
                                                    right: 0;
                                                    bottom: 0;
                                                    color: transparent;
                                                "
                                            />
                                        </div>
                                    </div>
                                    <div class="w-full">
                                        <h5>
                                            <a
                                                class="hover:text-primary dark:hover:text-primary mb-[6px] block text-base font-medium leading-snug text-black dark:text-white"
                                                href="#"
                                            >
                                                Best way to boost your online
                                                sales.
                                            </a>
                                        </h5>
                                        <p
                                            class="text-body-color text-xs font-medium"
                                        >
                                            12 Feb 2025
                                        </p>
                                    </div>
                                </div>
                            </li>
                            <li
                                class="border-body-color mb-6 border-b border-opacity-10 pb-6 dark:border-white dark:border-opacity-10"
                            >
                                <div class="flex items-center lg:block xl:flex">
                                    <div class="mr-5 lg:mb-3 xl:mb-0">
                                        <div
                                            class="relative h-[60px] w-[70px] overflow-hidden rounded-md sm:h-[75px] sm:w-[85px]"
                                        >
                                            <img
                                                alt="50 Best web design tips &amp; tricks that will help you."
                                                sizes="100vw"
                                                srcset="
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=640&amp;q=75   640w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=750&amp;q=75   750w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=828&amp;q=75   828w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=1080&amp;q=75 1080w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=1200&amp;q=75 1200w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=1920&amp;q=75 1920w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=2048&amp;q=75 2048w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=3840&amp;q=75 3840w
                                                "
                                                src="../_next/image/index76bb.html?url=%2Fimages%2Fblog%2Fpost-02.jpg&amp;w=3840&amp;q=75"
                                                decoding="async"
                                                data-nimg="fill"
                                                loading="lazy"
                                                style="
                                                    position: absolute;
                                                    height: 100%;
                                                    width: 100%;
                                                    left: 0;
                                                    top: 0;
                                                    right: 0;
                                                    bottom: 0;
                                                    color: transparent;
                                                "
                                            />
                                        </div>
                                    </div>
                                    <div class="w-full">
                                        <h5>
                                            <a
                                                class="hover:text-primary dark:hover:text-primary mb-[6px] block text-base font-medium leading-snug text-black dark:text-white"
                                                href="#"
                                            >
                                                50 Best web design tips &amp;
                                                tricks that will help you.
                                            </a>
                                        </h5>
                                        <p
                                            class="text-body-color text-xs font-medium"
                                        >
                                            15 Feb, 2024
                                        </p>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="flex items-center lg:block xl:flex">
                                    <div class="mr-5 lg:mb-3 xl:mb-0">
                                        <div
                                            class="relative h-[60px] w-[70px] overflow-hidden rounded-md sm:h-[75px] sm:w-[85px]"
                                        >
                                            <img
                                                alt="The 8 best landing page builders, reviewed"
                                                sizes="100vw"
                                                srcset="
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=640&amp;q=75   640w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=750&amp;q=75   750w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=828&amp;q=75   828w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=1080&amp;q=75 1080w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=1200&amp;q=75 1200w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=1920&amp;q=75 1920w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=2048&amp;q=75 2048w,
                                                    /_next/image/?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=3840&amp;q=75 3840w
                                                "
                                                src="../_next/image/index9fe9.html?url=%2Fimages%2Fblog%2Fpost-03.jpg&amp;w=3840&amp;q=75"
                                                decoding="async"
                                                data-nimg="fill"
                                                loading="lazy"
                                                style="
                                                    position: absolute;
                                                    height: 100%;
                                                    width: 100%;
                                                    left: 0;
                                                    top: 0;
                                                    right: 0;
                                                    bottom: 0;
                                                    color: transparent;
                                                "
                                            />
                                        </div>
                                    </div>
                                    <div class="w-full">
                                        <h5>
                                            <a
                                                class="hover:text-primary dark:hover:text-primary mb-[6px] block text-base font-medium leading-snug text-black dark:text-white"
                                                href="#"
                                            >
                                                The 8 best landing page
                                                builders, reviewed
                                            </a>
                                        </h5>
                                        <p
                                            class="text-body-color text-xs font-medium"
                                        >
                                            05 Jun, 2024
                                        </p>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div
                        class="bg-primary mb-10 rounded-md bg-opacity-5 dark:bg-opacity-10"
                    >
                        <h3
                            class="border-body-color border-b border-opacity-10 px-8 py-4 text-lg font-semibold text-black dark:border-white dark:border-opacity-10 dark:text-white"
                        >
                            Popular Category
                        </h3>
                        <ul class="px-8 py-6">
                            <li>
                                <a
                                    href="#0"
                                    class="text-body-color hover:text-primary mb-3 inline-block text-base font-medium"
                                >
                                    Tailwind Templates
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#0"
                                    class="text-body-color hover:text-primary mb-3 inline-block text-base font-medium"
                                >
                                    Landing page
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#0"
                                    class="text-body-color hover:text-primary mb-3 inline-block text-base font-medium"
                                >
                                    Startup
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#0"
                                    class="text-body-color hover:text-primary mb-3 inline-block text-base font-medium"
                                >
                                    Business
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#0"
                                    class="text-body-color hover:text-primary mb-3 inline-block text-base font-medium"
                                >
                                    Multipurpose
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div
                        class="bg-primary mb-10 rounded-md bg-opacity-5 dark:bg-opacity-10"
                    >
                        <h3
                            class="border-body-color border-b border-opacity-10 px-8 py-4 text-lg font-semibold text-black dark:border-white dark:border-opacity-10 dark:text-white"
                        >
                            Popular Tags
                        </h3>
                        <div class="flex flex-wrap px-8 py-6">
                            <a
                                href="#0"
                                class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                            >
                                Themes
                            </a>
                            <a
                                href="#0"
                                class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                            >
                                UI Kit
                            </a>
                            <a
                                href="#0"
                                class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                            >
                                Tailwind
                            </a>
                            <a
                                href="#0"
                                class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                            >
                                Startup
                            </a>
                            <a
                                href="#0"
                                class="bg-primary text-body-color mb-3 mr-3 inline-flex items-center justify-center rounded-md bg-opacity-10 px-4 py-2 duration-300 hover:bg-opacity-100 hover:text-white"
                            >
                                Business
                            </a>
                        </div>
                    </div>
                    <div
                        class="wow fadeInUp bg-primary/[3%] dark:bg-primary/10 relative z-10 rounded-md p-8 sm:p-11 lg:p-8 xl:p-11"
                        data-wow-delay=".2s"
                    >
                        <h3
                            class="mb-4 text-2xl font-bold leading-tight text-black dark:text-white"
                        >
                            Subscribe to receive future updates
                        </h3>
                        <p
                            class="border-body-color text-body-color mb-11 border-b border-opacity-25 pb-11 text-base font-medium leading-relaxed dark:border-white dark:border-opacity-25"
                        >
                            Lorem ipsum dolor sited Sed ullam corper consectur
                            adipiscing Mae ornare massa quis lectus.
                        </p>
                        <form>
                            <input
                                type="text"
                                name="name"
                                placeholder="Enter your name"
                                class="border-body-color text-body-color placeholder-body-color focus:border-primary mb-4 w-full rounded-md border border-opacity-10 px-6 py-3 text-base font-medium outline-none focus:border-opacity-100 focus-visible:shadow-none dark:border-white dark:border-opacity-10 dark:bg-[#242B51] focus:dark:border-opacity-50"
                            />
                            <input
                                type="email"
                                name="email"
                                placeholder="Enter your email"
                                class="border-body-color text-body-color placeholder-body-color focus:border-primary mb-4 w-full rounded-md border border-opacity-10 px-6 py-3 text-base font-medium outline-none focus:border-opacity-100 focus-visible:shadow-none dark:border-white dark:border-opacity-10 dark:bg-[#242B51] focus:dark:border-opacity-50"
                            />
                            <input
                                type="submit"
                                class="duration-80 bg-primary hover:shadow-signUp mb-4 w-full cursor-pointer rounded-md border border-transparent px-6 py-3 text-center text-base font-medium text-white outline-none transition ease-in-out hover:bg-opacity-80 focus-visible:shadow-none"
                                value="Subscribe"
                            />
                            <p
                                class="text-body-color text-center text-base font-medium leading-relaxed"
                            >
                                No spam guaranteed, So please don’t send any
                                spam mail.
                            </p>
                        </form>
                        <div class="absolute left-0 top-0 z-[-1]">
                            <svg
                                width="370"
                                height="596"
                                viewBox="0 0 370 596"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <mask
                                    id="mask0_88:141"
                                    style="mask-type: alpha"
                                    maskUnits="userSpaceOnUse"
                                    x="0"
                                    y="0"
                                    width="370"
                                    height="596"
                                >
                                    <rect
                                        width="370"
                                        height="596"
                                        rx="2"
                                        fill="#1D2144"
                                    ></rect>
                                </mask>
                                <g mask="url(#mask0_88:141)">
                                    <path
                                        opacity="0.15"
                                        d="M15.4076 50.9571L54.1541 99.0711L71.4489 35.1605L15.4076 50.9571Z"
                                        fill="url(#paint0_linear_88:141)"
                                    ></path>
                                    <path
                                        opacity="0.15"
                                        d="M20.7137 501.422L44.6431 474.241L6 470.624L20.7137 501.422Z"
                                        fill="url(#paint1_linear_88:141)"
                                    ></path>
                                    <path
                                        opacity="0.1"
                                        d="M331.676 198.309C344.398 204.636 359.168 194.704 358.107 180.536C357.12 167.363 342.941 159.531 331.265 165.71C318.077 172.69 318.317 191.664 331.676 198.309Z"
                                        fill="url(#paint2_linear_88:141)"
                                    ></path>
                                    <g opacity="0.3">
                                        <path
                                            d="M209 89.9999C216 77.3332 235.7 50.7999 258.5 45.9999C287 39.9999 303 41.9999 314 30.4999C325 18.9999 334 -3.50014 357 -3.50014C380 -3.50014 395 4.99986 408.5 -8.50014C422 -22.0001 418.5 -46.0001 452 -37.5001C478.8 -30.7001 515.167 -45 530 -53"
                                            stroke="url(#paint3_linear_88:141)"
                                        ></path>
                                        <path
                                            d="M251 64.9999C258 52.3332 277.7 25.7999 300.5 20.9999C329 14.9999 345 16.9999 356 5.49986C367 -6.00014 376 -28.5001 399 -28.5001C422 -28.5001 437 -20.0001 450.5 -33.5001C464 -47.0001 460.5 -71.0001 494 -62.5001C520.8 -55.7001 557.167 -70 572 -78"
                                            stroke="url(#paint4_linear_88:141)"
                                        ></path>
                                        <path
                                            d="M212 73.9999C219 61.3332 238.7 34.7999 261.5 29.9999C290 23.9999 306 25.9999 317 14.4999C328 2.99986 337 -19.5001 360 -19.5001C383 -19.5001 398 -11.0001 411.5 -24.5001C425 -38.0001 421.5 -62.0001 455 -53.5001C481.8 -46.7001 518.167 -61 533 -69"
                                            stroke="url(#paint5_linear_88:141)"
                                        ></path>
                                        <path
                                            d="M249 40.9999C256 28.3332 275.7 1.79986 298.5 -3.00014C327 -9.00014 343 -7.00014 354 -18.5001C365 -30.0001 374 -52.5001 397 -52.5001C420 -52.5001 435 -44.0001 448.5 -57.5001C462 -71.0001 458.5 -95.0001 492 -86.5001C518.8 -79.7001 555.167 -94 570 -102"
                                            stroke="url(#paint6_linear_88:141)"
                                        ></path>
                                    </g>
                                </g>
                                <defs>
                                    <linearGradient
                                        id="paint0_linear_88:141"
                                        x1="13.4497"
                                        y1="63.5059"
                                        x2="81.144"
                                        y2="41.5072"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint1_linear_88:141"
                                        x1="28.1579"
                                        y1="501.301"
                                        x2="8.69936"
                                        y2="464.391"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint2_linear_88:141"
                                        x1="338"
                                        y1="167"
                                        x2="349.488"
                                        y2="200.004"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint3_linear_88:141"
                                        x1="369.5"
                                        y1="-53"
                                        x2="369.5"
                                        y2="89.9999"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint4_linear_88:141"
                                        x1="411.5"
                                        y1="-78"
                                        x2="411.5"
                                        y2="64.9999"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint5_linear_88:141"
                                        x1="372.5"
                                        y1="-69"
                                        x2="372.5"
                                        y2="73.9999"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="paint6_linear_88:141"
                                        x1="409.5"
                                        y1="-102"
                                        x2="409.5"
                                        y2="40.9999"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop stop-color="white"></stop>
                                        <stop
                                            offset="1"
                                            stop-color="white"
                                            stop-opacity="0"
                                        ></stop>
                                    </linearGradient>
                                </defs>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>
