            ...
            <form method="post" class="mt-6 space-y-6" wire:submit="update">
                <div class='max-w-md space-y-6'>
                    ...
                    <div>
                        ...
                        {{-- image preview --}}
                        @if ($old_image && !$image)
                            <img src="{{ asset('storage/' . $old_image) }}" alt="image"
                        ...
                        <x-input  type="file" placeholder="Post image" class="mt-2"
                            wire:model.live="image" :value="old('image')" />
                    </div>
                    {{-- categories --}}
                ...
                <div class="space-y-6 ">
                    ...
                    <div wire:ignore>
                        ...
                        <x-ck-textarea id="description" name='description' wire:model="description"
                        :value="old('description')" class="block w-full mt-1" />
                            ...
                            <div class="w-full mb-4 lg:w-1/3">
                                ...
                                @foreach ($tags as $key => $tag)
                                    <x-checkbox  label="{{ $tag['name'] }}" value="{{ $tag['id'] }}"
                                        wire:model="tags"   :key="$tag['id']"  :id="$tag['id']" />
                                @endforeach
                            </div>
                        @endforeach
                    </div>
